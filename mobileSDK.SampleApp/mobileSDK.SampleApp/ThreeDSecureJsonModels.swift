...
struct ThreeDSecureInfo: Codable {
    ...
    var sdkThreeDSecureInfo: flashpaySDK_Dev.ThreeDSecureInfo {
        flashpaySDK_Dev.ThreeDSecureInfo.init(
            threeDSecurePaymentInfo: paymentMerchantRisk?.payment.map({ payment in
                flashpaySDK_Dev.ThreeDSecurePaymentInfo(
                    ...
                    giftCard: payment.giftCard.map({ giftCard in
                        flashpaySDK_Dev.ThreeDSecureGiftCardInfo(
            ...
            threeDSecureCustomerInfo: flashpaySDK_Dev.ThreeDSecureCustomerInfo(
                ...
                accountInfo: customerAccountInfo?.customer?.account.map({ account in
                    flashpaySDK_Dev.ThreeDSecureAccountInfo(
                ...
                shippingInfo: customerShipping?.customer?.shipping.map({ shipping in
                    flashpaySDK_Dev.ThreeDSecureShippingInfo(
                        type: shipping.type,
                ...
                mpiResultInfo: customerMpiResult?.customer?.mpiResult.map({ mpiResult in
                    flashpaySDK_Dev.ThreeDSecureMpiResultInfo(
